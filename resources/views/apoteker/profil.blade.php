...
<style>
    ...
    .id-card{
        background:linear-gradient(150deg, var(--mint-500) 0%, var(--mint-700) 100%);
        border-radius:24px;
        padding:30px 24px;
        color:#fff;
        text-align:center;
        box-shadow:0 8px 24px rgba(15,47,34,.1);
    }
    .id-card .avatar-big{
...
<div class="profile-grid">
    <div class="id-card">
        <div class="avatar-big">{{ strtoupper(substr($user->name ?? 'A', 0, 1)) }}</div>
        <h2>{{ $user->name ?? '-' }}</h2>
        <div class="id-email">{{ $user->email ?? '-' }}</div>
        <span class="id-tag">{{ $user->role ?? 'Apoteker' }}</span>
        <div class="id-shift">🕐 Shift: {{ $user->shiftLabel() }}</div>
        <div class="shift-note">Shift diatur oleh pemilik apotek</div>

        <form method="POST" action="{{ route('staff.logout') }}" class="logout-form">
            @csrf
    ...
    <div>
        <div class="card">
            <div class="card-heading">
                ...
                <button type="button" class="btn-edit-toggle" id="toggleEditBtn" onclick="toggleEdit()">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 20h9"/><path d="M16.5 3.5a2.12 2.12 0 0 1 3 3L7 19l-4 1 1-4Z"/></svg>
                    <span id="toggleEditText">Edit</span>
                </button>
            </div>

            <form method="POST" action="{{ route('apoteker.profil.update') }}" id="profileForm">
                @csrf
                ...
                <div class="info-static-row">
                    ...
                    <span class="value">{{ $user->created_at ? $user->created_at->translatedFormat('d F Y') : '-' }}</span>
                </div>
        ...
        <div class="card">
            ...
            <form method="POST" action="{{ route('apoteker.profil.password') }}">
                @csrf
